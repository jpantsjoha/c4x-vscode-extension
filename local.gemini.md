...
## 🧠 AI Model Strategy (v1.1.5)

**Primary**: `gemini-3-pro-preview` (Coding & Complex Reasoning)
**Fallback**: `gemini-2.5-pro` (Production Stability)
...
**Implementation Note**:
`GeminiService.ts` implements a silent fallback mechanism. If `gemini-3-pro-preview` fails (API error or validation exhaustion), it auto-switches to `gemini-2.5-pro`. This ensures reliability while offering state-of-the-art capabilities.
